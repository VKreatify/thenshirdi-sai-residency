...
export default function BentoCard({
  ...
  const handleMouseMove = (e) => {
    ...
    if (!tilt || isTouchDevice || !cardRef.current) return;
    ...
    >
      ...
      <div
        style={{
          ...
          background: `radial-gradient(600px circle at ${glowPos.x}% ${glowPos.y}%, ${dark ? 'rgba(201, 160, 99, 0.18)' : 'rgba(201, 160, 99, 0.14)'}, transparent 40%)`,
          pointerEvents: 'none',
          zIndex: 1,
      ...
      />
      <div style={{ position: 'relative', zIndex: 2, transform: isHovered ? 'translateZ(20px)' : 'translateZ(0px)', transition: 'transform 0.35s ease' }}>
        {children}
      </div>
    </div>
  );
